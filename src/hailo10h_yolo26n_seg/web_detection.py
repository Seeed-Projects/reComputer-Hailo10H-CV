...
parser.add_argument("--model_path", default=os.path.join(os.path.dirname(__file__), "model", "yolo26n_seg.hef"))
...
@app.get("/health")
async def health():
    return {"status": "ok", "model": "yolo26n_seg", "model_loaded": infer is not None}

@app.get("/api/models/yolo26n_seg/classes")
async def get_classes():
    return {"classes": [{"id": i, "name": n} for i, n in enumerate(COCO_CLASSES)]}

@app.post("/api/models/yolo26n_seg/predict")
...
@app.post("/api/models/yolo26n_seg/visualize")
