...
# Demo defaults. Model Zoo v5.4.0 evaluates at score_threshold=0.1 and the
...
# tiny_yolov4_license_plates anchors from Model Zoo v5.4.0.
...
app = FastAPI(title="reComputer Tiny-YOLOv4 License Plates Hailo-10H")
...
async def index():
    ...
    <html><head><title>reComputer License Plate Detection · Hailo-10H</title>
    <style>
    ...
    <div class="container"><h1>Tiny-YOLOv4 License Plates · CM5 + Hailo-10H</h1>
...
def _collect_yolo_heads(hailo_output):
    ...
    # Older Hailo-10H HEFs can expose centers/scales/objectness/probabilities as
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    if not _DET_OUTPUT_LOGGED:
        # One-shot output-range log (SOP §14 acceptance): the decoded heads
        # must be logits (±~10) — values in the tens of thousands mean the
        # executor read the quantized encoding instead of FLOAT32.
        for i, h in enumerate(heads):
            print(f"[Tiny-YOLOv4-License-Plates] head{i}: shape={h.shape} "
                  f"range=[{h.min():.2f}, {h.max():.2f}]", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description='Tiny-YOLOv4 license-plate detection on CM5 + Hailo-10H')
