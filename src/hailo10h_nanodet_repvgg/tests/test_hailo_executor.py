...
class _HEF:
    output_specs = [("nanodet_repvgg/nms_postprocess", [80, 5, 100])]
...
class _InferModel:
    ...
    def input(self):
        return _ModelInfo("nanodet_repvgg/input_layer1", [320, 320, 3])
...
class HailoExecutorTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        _HEF.output_specs = [("nanodet_repvgg/nms_postprocess", [80, 5, 100])]

    def test_configuration_and_bindings_are_reused_across_frames(self):
        infer = self.executor.HailoInfer("model/nanodet_repvgg.hef")
        ...
        self.assertEqual(set(configured.output_buffers), {"nanodet_repvgg/nms_postprocess"})
        self.assertEqual(
            configured.output_buffers["nanodet_repvgg/nms_postprocess"].shape,
            (80, 5, 100),
        )
        self.assertEqual(
            configured.output_buffers["nanodet_repvgg/nms_postprocess"].dtype,
            np.dtype("float32"),
        )
        self.assertIsNot(
            first_outputs["nanodet_repvgg/nms_postprocess"],
            configured.output_buffers["nanodet_repvgg/nms_postprocess"],
        )
        np.testing.assert_array_equal(first_outputs["nanodet_repvgg/nms_postprocess"], 1)
        np.testing.assert_array_equal(second_outputs["nanodet_repvgg/nms_postprocess"], 2)
