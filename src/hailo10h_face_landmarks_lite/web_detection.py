...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (192, 192)  # (width, height) must match the .hef input shape
...
@app.post("/api/models/face_landmarks_lite/predict")
async def predict(
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Extract facial keypoints from heatmaps."""
    global _SEG_OUTPUT_LOGGED
    if hailo_output is None:
        return None
    raw = np.asarray(_primary_output_tensor(hailo_output))
    if raw.ndim == 3 and raw.shape[0] == 1:
        raw = raw[0]
    if raw.shape[0] >= 98 * 2:
        kps = raw[:98*2].reshape(98, 2)
        confs = np.ones(98)
    else:
        kps = raw.reshape(-1, 2)
        confs = np.ones(len(kps))
    if not _SEG_OUTPUT_LOGGED:
        print(f"[Face Landmarks] {len(kps)} keypoints", flush=True)
...
def draw_landmarks(image, kps, confs, lb_info=None):
    if kps is None:
        return
    h, w = image.shape[:2]
