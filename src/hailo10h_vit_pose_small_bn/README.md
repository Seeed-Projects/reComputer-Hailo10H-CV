# ViTPose-Small-BN on CM5 + Hailo-10H

ViTPose-Small-BN performs single-person 2D pose estimation with 17 COCO keypoints. The
...
```bash
sudo docker build -f docker/hailo10h/vit_pose_small_bn.dockerfile \
  -t vit_pose_small_bn:latest \
  src/hailo10h_vit_pose_small_bn
```
...
```bash
sudo docker run --rm \
  --name cm5-hailo10h-vit-pose-small-bn \
  --privileged \
  --net=host \
  ...
  ghcr.io/seeed-projects/recomputer-hailo10h-cv/vit_pose_small_bn:latest \
...
|---|---|---|
| `/` | GET | Web preview UI |
| `/api/models/vit_pose_small_bn/predict` | POST | 17 COCO keypoints (JSON) |
...
4. `POST /api/models/vit_pose_small_bn/predict` returns 17 keypoints with scores.
