# CM5 + Hailo-10H 上的 ViTPose-Small-BN

ViTPose-Small-BN 用于单人 2D 姿态估计，输出 17 个 COCO 关键点。HEF 输出单张热图
...
```bash
curl -X POST "http://<开发板IP>:8000/api/models/vit_pose_small_bn/predict" \
  -F "file=@test.jpg"
```
...
4. `POST /api/models/vit_pose_small_bn/predict` 返回 17 个带分数的关键点。
