# CM5 + Hailo-10H LPRNet 车牌识别流水线

本模块采用两级流水线：Tiny-YOLOv4 从整帧检测车牌，再把原始分辨率车牌区域交给 LPRNet 识别数字。两个 HEF 通过 HailoRT round-robin 调度和共享 VDevice 组使用同一块 Hailo-10H。
...
本地已完成 Python 语法和素材检查；双 HEF 调度、真实 tensor、OCR 结果、Docker 和端到端效果仍需 CM5 + Hailo-10H 实机验收。
