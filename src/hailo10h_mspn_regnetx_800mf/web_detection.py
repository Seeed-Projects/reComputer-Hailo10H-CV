...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (192, 256)  # (width, height) must match the .hef input shape
            ...
            while not self._stop_event.is_set():
                ...
                if self.model and self.co_helper:
                    ...
                    if outputs is not None:
                        ...
                        keypoints = post_process_hailo(outputs, 0, 0, IMG_SIZE[1], IMG_SIZE[0])
                        if keypoints is not None:
                            draw_pose(frame, keypoints, lb_info)
                if kind == 'ffmpeg':
...
app = FastAPI(title="reComputer Hailo-CV Web Preview (CM5 + Hailo-10H)")
...
async def index():
    ...
    <html>
      ...
      <body>
        <div class="container">
          <h1>CM5 + Hailo-10H Real-time Detection</h1>
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Extract 17 COCO keypoints from heatmaps."""
    global _SEG_OUTPUT_LOGGED
    if hailo_output is None:
        return None
    heatmaps = np.asarray(_primary_output_tensor(hailo_output))
    if heatmaps.ndim == 4:
        heatmaps = heatmaps[0]
    keypoints = []
    for k in range(17):
    ...
    if not _SEG_OUTPUT_LOGGED:
        print(f"[MSPN] extracted {len(keypoints)} keypoints", flush=True)
...
def draw_pose(image, keypoints, lb_info=None):
    if keypoints is None:
        return
    h, w = image.shape[:2]
    COCO_SKELETON = [(15,13),(13,11),(16,14),(14,12),(11,12),(5,11),(6,12),(5,6),(5,7),(6,8),(7,9),(8,10),(1,2),(0,1),(0,2),(1,3),(2,4),(3,5),(4,6)]
    KEYPOINT_COLORS = [(255,0,0),(0,255,0),(0,0,255),(255,255,0),(0,255,255),(255,0,255),(128,255,0),(255,128,0),(0,128,255),(128,0,255),(255,255,128),(128,255,255),(255,128,255),(0,255,128),(128,128,255),(255,128,128),(128,255,128)]
    for i, kp in enumerate(keypoints):
        if kp[2] > 0.3:
            cv2.circle(image, (int(kp[0]*w), int(kp[1]*h)), 4, KEYPOINT_COLORS[i], -1)
    for a, b in COCO_SKELETON:
        if keypoints[a][2] > 0.3 and keypoints[b][2] > 0.3:
            cv2.line(image, (int(keypoints[a][0]*w), int(keypoints[a][1]*h)), (int(keypoints[b][0]*w), int(keypoints[b][1]*h)), (0,255,0), 2)
