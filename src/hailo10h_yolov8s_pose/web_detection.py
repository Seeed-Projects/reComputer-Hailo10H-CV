...
# YOLOv8 Pose has a single class and appends 17 COCO keypoints to every NMS
...
# YOLOv8 Pose is a single-class model: every NMS row is a person.
DEFAULT_CLASSES = (
    "person",  # 0
)
...
app = FastAPI(title="reComputer YOLOv8 Pose Hailo-10H")
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            for idx, (box, score, cl) in enumerate(zip(real_boxes, scores, classes)):
                ...
                if cl < 0 or cl >= len(CLASSES) or CLASSES[cl] == "N/A":
                    continue
                points = []
                if real_kpts is not None and idx < len(real_kpts):
                    points = [
                        {"x": int(x), "y": int(y), "score": float(s)}
                        for x, y, s in real_kpts[idx]
                        if np.isfinite(x) and np.isfinite(y)
                        and float(s) >= KEYPOINT_SCORE_THRESH
                    ]
                predictions.append({
                    "class": CLASSES[cl],
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer YOLOv8 Pose · Hailo-10H</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>YOLOv8 Pose · CM5 + Hailo-10H</h1>
          <div class="tabs">
...
def unletterbox_keypoints(keypoints, lb_info):
    """Map keypoints (N, 17, 3) from the letterboxed input back to the frame."""
...
def draw(image, boxes, scores, classes, keypoints=None):
    for idx, (box, score, cl) in enumerate(zip(boxes, scores, classes)):
        ...
        if keypoints is None or idx >= len(keypoints):
            continue
        pts = np.asarray(keypoints[idx])
        if pts.ndim != 2 or pts.shape[0] < NUM_KEYPOINTS:
            continue
        visible = (np.isfinite(pts[:, 0]) & np.isfinite(pts[:, 1])
                   & (pts[:, 2] >= KEYPOINT_SCORE_THRESH))
        ...
        for kp_idx in range(NUM_KEYPOINTS):
            if visible[kp_idx]:
                ...
                cv2.circle(image, (int(pts[kp_idx, 0]), int(pts[kp_idx, 1])),
                           3, color, -1)
...
def main():
    parser = argparse.ArgumentParser(description='YOLOv8 Pose on CM5 + Hailo-10H (Web Preview Mode)')
