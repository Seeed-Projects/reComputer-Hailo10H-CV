...
# Mock output specs shaped like the CenterPose HEF vstreams (six heads):
#   hm(1), wh(2), hps(34), reg(2), hm_hp(17), hp_offset(2) at 128x128
DEFAULT_OUTPUT_SPECS = [
    ("centerpose_regnetx_800mf/conv60", [128, 128, 1]),
    ("centerpose_regnetx_800mf/conv61", [128, 128, 2]),
    ("centerpose_regnetx_800mf/conv62", [128, 128, 34]),
    ("centerpose_regnetx_800mf/conv63", [128, 128, 2]),
    ("centerpose_regnetx_800mf/conv64", [128, 128, 17]),
    ("centerpose_regnetx_800mf/conv65", [128, 128, 2]),
]
...
class _InferModel:
    ...
    def input(self):
        return _ModelInfo("centerpose_regnetx_800mf/input_layer1", [512, 512, 3])
...
class HailoExecutorTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        spec = importlib.util.spec_from_file_location(
            "centerpose_hailo_executor", EXECUTOR_PATH
        )
    ...
    def setUp(self):
        ...
        _HEF.output_specs = [("centerpose_regnetx_800mf/conv60", [128, 128, 1])]

    def test_configuration_and_bindings_are_reused_across_frames(self):
        infer = self.executor.HailoInfer("model/centerpose_regnetx_800mf.hef")
        ...
        self.assertEqual(
            set(configured.output_buffers), {"centerpose_regnetx_800mf/conv60"}
        )
        self.assertEqual(
            configured.output_buffers["centerpose_regnetx_800mf/conv60"].shape,
            (128, 128, 1),
        )
        ...
        self.assertEqual(
            configured.output_buffers["centerpose_regnetx_800mf/conv60"].dtype,
            np.dtype("float32"),
        )
        self.assertIsNot(
            first_outputs["centerpose_regnetx_800mf/conv60"],
            configured.output_buffers["centerpose_regnetx_800mf/conv60"],
        )
        np.testing.assert_array_equal(
            first_outputs["centerpose_regnetx_800mf/conv60"], 1
        )
        np.testing.assert_array_equal(
            second_outputs["centerpose_regnetx_800mf/conv60"], 2
        )
    ...
    def test_six_outputs_use_one_persistent_binding_set(self):
