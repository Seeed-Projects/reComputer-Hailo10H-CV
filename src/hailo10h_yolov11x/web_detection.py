...
# YOLOv11x outputs the standard continuous 80 COCO classes (base config:
...
app = FastAPI(title="reComputer YOLOv11x Hailo-10H")
...
@app.post("/api/models/yolov11x/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer YOLOv11x · Hailo-10H</title>
        <style>
...
# ---------------------------------------------------------------------------
# YOLOv11x post-processing (on-chip NMS / HPP)
#
...
def main():
    parser = argparse.ArgumentParser(description='YOLOv11x on CM5 + Hailo-10H (Web Preview Mode)')
