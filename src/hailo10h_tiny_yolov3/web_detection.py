...
OBJ_THRESH = 0.10  # official eval score_threshold
NMS_THRESH = 0.30
...
app = FastAPI(title="reComputer Tiny-YOLOv3 Hailo-10H")
...
async def index():
    ...
    <html><head><title>reComputer Tiny-YOLOv3 · Hailo-10H</title>
    <style>
    ...
    <div class="container"><h1>Tiny-YOLOv3 · CM5 + Hailo-10H</h1>
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    if not _DET_OUTPUT_LOGGED:
        if not _DET_OUTPUT_LOGGED:
            ...
            # FLOAT32 logits (±~20 with the alls clipping values) —
            # tens-of-thousands values mean the quantized encoding leaked.
            for i, h in enumerate(heads):
                print(f"[tiny_yolov3] head{i}: shape={h.shape} "
...
def unletterbox_boxes(boxes, lb_info):
    ...
    # Padding is bottom/right only: content sits at the origin, so the
...
def preprocess_frame(frame, co_helper=None):
    ...
    NO input_conversion(bgr_to_rgb) and normalization(0, 255) — the HEF
    expects raw uint8 BGR. (LPR lesson: do not letterbox with gray 114 or
    convert to RGB here.)
    """
...
def main():
    parser = argparse.ArgumentParser(description='Tiny-YOLOv3 on CM5 + Hailo-10H (Web Preview Mode)')
