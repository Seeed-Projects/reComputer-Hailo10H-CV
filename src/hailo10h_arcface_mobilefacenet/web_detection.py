...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (112, 112)  # (width, height) must match the .hef input shape
...
async def predict(
    ...
    try:
        ...
        if mask is not None:
            ...
            predictions.append({"embedding": embedding.tolist(), "dim": len(embedding)})
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    if hailo_output is None:
        return None
    embedding = np.asarray(_primary_output_tensor(hailo_output)).flatten()
    if not _SEG_OUTPUT_LOGGED:
        print(f"[ArcFace] embedding dim: {len(embedding)}", flush=True)
