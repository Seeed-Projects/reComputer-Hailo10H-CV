...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (320, 240)  # (width, height) must match the .hef input shape
            ...
            while not self._stop_event.is_set():
                ...
                if self.model and self.co_helper:
                    ...
                    if outputs is not None:
                        ...
                        boxes, confs = post_process_hailo(outputs, 0, 0, IMG_SIZE[1], IMG_SIZE[0])
                        if boxes is not None:
                            draw_boxes(frame, boxes, confs, lb_info)
                if kind == 'ffmpeg':
...
async def predict(
    ...
    try:
        ...
        predictions = []
        if boxes is not None:
            draw_boxes(img, boxes, confs, lb_info)
            predictions = [
                {
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Extract face bounding boxes from detection output."""
    ...
    if not _SEG_OUTPUT_LOGGED:
        print(f"[LightFace] detected {len(boxes)} faces", flush=True)
...
def draw_boxes(image, boxes, confs, lb_info=None):
    if boxes is None:
        return
    h, w = image.shape[:2]
    for i, box in enumerate(boxes):
        ...
        cv2.putText(image, f"{confs[i]:.2f}", (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
...
def main():
    parser = argparse.ArgumentParser(description='LightFace Slim face detection on CM5 + Hailo-10H (Web Preview Mode)')
