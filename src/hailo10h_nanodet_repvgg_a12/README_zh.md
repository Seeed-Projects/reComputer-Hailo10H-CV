# NanoDet-RepVGG-a12：reComputer CM5 + Hailo-10H 目标检测
...
从已验证的 Hailo-8 模块 `rpi5_hailo8_nanodet_repvgg_a12` 移植；执行器替换为
...
```bash
sudo docker build -f docker/hailo10h/nanodet_repvgg_a12.dockerfile \
  -t nanodet_repvgg_a12:latest \
  src/hailo10h_nanodet_repvgg_a12
```
...
```bash
sudo docker run --rm \
  --name cm5-hailo10h-nanodet-repvgg-a12 \
  --privileged \
  --net=host \
  ...
  ghcr.io/seeed-projects/recomputer-hailo10h-cv/nanodet_repvgg_a12:latest \
...
```bash
curl -X POST "http://<CM5_IP>:8000/api/models/nanodet_repvgg_a12/predict" \
  -F "file=@test.jpg"
```
...
`nanodet_repvgg_a12.hef` 为 Hailo Model Zoo v5.4.0 的 Hailo-10H 构建——
